            ...
            $datatable = Datatables::of($products)
                ->editColumn('stock', function ($row) {
                    if ($row->enable_stock) {
                        ...
                        // Convert the string to an array if it's in JSON format
                        if (!empty($row->sub_unit_ids) && $stock > 0) {
                            // Decode the JSON string to an array
                            $sub_unit_ids = json_decode($row->sub_unit_ids, true);
                    
                            // Ensure $sub_unit_ids is now an array before processing
                            if (is_array($sub_unit_ids)) {
                                if (in_array('4', $sub_unit_ids)) {
                                    $casier = 12;
                                } elseif (in_array('5', $sub_unit_ids)) {
                                    $casier = 20;
                                } elseif (in_array('7', $sub_unit_ids)) {
                                    $casier = 24;
                                }
                            }
